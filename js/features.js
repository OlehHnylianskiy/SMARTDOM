...
// Реєстрація входу гостя
function registerGuestEntry() {
  if (!currentUser) {
  ...
  const guestSurname = document.getElementById('guest-surname').value;
  const guestRoom = document.getElementById('guest-room').value;

  if (!guestName || !guestSurname || !guestRoom) {
    showAlert('Заповни ім\'я, прізвище та кімнату гостя!', 'error');
    return;
  }
  ...
  db.collection('users').doc(currentUser.uid).get().then(doc => {
    if (doc.exists) {
      ...
      // Записуємо вхід гостя з його кімнатою
      ...
      showAlert(`✓ Гість ${guestName} ${guestSurname} (кім. ${guestRoom}) зареєстрований!`, 'success');
      document.getElementById('guest-name').value = '';
      document.getElementById('guest-surname').value = '';
      document.getElementById('guest-room').value = '';
      
      // Оновлюємо журнал входів
      loadAccessLogs();
    }
  }).catch(error => {
...
function loadRepairRequests() {
  ...
  db.collection('repair_requests').onSnapshot(snapshot => {
    ...
    if (table) {
      ...
      // Фільтруємо на клієнті та сортуємо
      const myRequests = [];
      snapshot.forEach(doc => {
        const data = doc.data();
        if (data.userId === currentUser.uid) {
          myRequests.push({ ...data, id: doc.id });
        }
      });
      ...
      myRequests.forEach(data => {
        const date = data.createdAt ? new Date(data.createdAt.toDate()).toLocaleDateString('uk-UA') : 'Невідомо';
        ...
        row.innerHTML = `
          ...
          <td>${date}</td>
          <td>
            ${data.status === 'new' ? `<button class="btn btn-secondary" onclick="cancelRepairRequest('${data.id}')">Скасувати</button>` : '-'}
          </td>
        `;
      });
    }
          ...
          db.collection('users').doc(data.userId).get().then(userDoc => {
            if (userDoc.exists) {
              ...
              row.innerHTML = `
                ...
                <td>${date}</td>
                <td>
                  <select onchange="updateRepairStatus('${data.id}', this.value)">
...
function loadPayments() {
  ...
  db.collection('payments').onSnapshot(snapshot => {
    ...
    if (table) {
      ...
      // Фільтруємо та сортуємо на клієнті
      const myPayments = [];
      snapshot.forEach(doc => {
        const data = doc.data();
        if (data.userId === currentUser.uid) {
      ...
      // Сортуємо по даті (новіші першими) та беремо останні 20
      myPayments.sort((a, b) => {
        const dateA = a.createdAt ? new Date(a.createdAt.toDate()) : new Date(0);
        const dateB = b.createdAt ? new Date(b.createdAt.toDate()) : new Date(0);
        return dateB - dateA;
      ...
      let totalSpent = 0;
      myPayments.slice(0, 20).forEach(data => {
        const date = data.createdAt ? new Date(data.createdAt.toDate()).toLocaleString('uk-UA') : 'Невідомо';

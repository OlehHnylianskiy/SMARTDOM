...
function generateDormReport() {
  Promise.all([
    ...
    // Фільтруємо студентів на клієнті
    ...
    const report = {
      ...
      totalPayments: paymentsSnap.docs.reduce((sum, d) => sum + (d.data().amount || 0), 0),
      averageBalance: students.length > 0 ? Array.from(students).reduce((sum, d) => sum + (d.data().balance || 0), 0) / students.length : 0
    };
